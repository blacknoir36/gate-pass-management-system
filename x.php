...
if (isset($_GET['search']) && isset($_GET['table'])) {
    $search = $_GET['search'];
    $table = $_GET['table'];
    $query = "SELECT * FROM $table WHERE first_name LIKE :search";
...
<body>
    <h1>Search Page (STUDENTS)</h1>
    <form id="searchform" method="GET">
        <input type="text" id="search-input" name="search" placeholder="Enter first name">
        <select name="table">
            <option value="certificate">Non-Degree</option>
            <option value="bachelor">Under Graduate</option>
